...
var environment = function(_plankImage){
	...
	var attackHouse = function(_window, damage){
		...
		//broken checks if the window breaks, and if it does, broadcast the broken window number
		var broken = false;
		//the plankNo remembers which plank is broken and broadcasts the 
		//broken plank to all the other players
		var plankNo = 0;
		...
		if(broken){//if a window breaks, broadcast its number to all players
			window.emitPlankBreak(plankNo);
		}
	...
	var repair = function(_window){
		//plankNo remembers which number plank is fixed, so the number is broadcast to all other clients
		var plankNo = 0;
		...
		//if a plank is repaired, broadcast the number to all other players
		emitPlankRepair(plankNo);
	};
